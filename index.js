  ...
  try {
    ...
    if (error.code === "auth/id-token-expired") {
      return res
        .status(401)
        .send({
          message: "Unauthorized: Token expired. Please re-authenticate.",
        });
    }
  ...
  try {
    ...
    app.get("/user/:id", async (req, res) => {
      try {
        ...
        res
          .status(500)
          .send({ message: "Failed to fetch user data", error: error.message });
      }
    });
    ...
    app.get("/allusers", verifyFirebaseToken, async (req, res) => {
      ...
      try {
        ...
        if (!dbUser) {
          console.warn(
            `Unauthorized access attempt to /allusers by UID: ${
              userFromToken.uid || "N/A"
            }`
          );
          return res
            .status(403)
            .send({ message: "Forbidden: You do not have admin access." });
        }
    ...
    app.get("/allusers-count", verifyFirebaseToken, async (req, res) => {
      try {
        ...
        if (!dbUser) {
          console.warn(
            `Unauthorized access attempt to /admin/stats/users by UID: ${
              userFromToken.uid || "N/A"
            }`
          );
          return res
            .status(403)
            .send({ message: "Admin/Volunteer User not logged in." });
        }
        ...
        console.error(
          "Error fetching total users count for admin dashboard:",
          error
        );
        res
          .status(500)
    ...
    app.post("/Users", async (req, res) => {
      try {
        ...
        if (existingUser) {
          console.warn(
            `User with UID ${userData.uid} already exists in DB. Not inserting again.`
          );
          return res
            .status(200)
            .json({ message: "User data already exists", data: existingUser });
        }
        ...
        res
          .status(500)
          .json({
            message: "Failed to save new user data",
            error: error.message,
          });
      }
    });
    ...
    app.post("/save-donation", async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({
            message: "Failed to save new Donation data",
            error: error.message,
          });
      }
    });
    ...
    app.get("/get-user-role", verifyFirebaseToken, async (req, res) => {
      try {
        ...
        if (user) {
          ...
          console.warn(
            `User with email ${req.firebaseUser.email} not found in DB. Assigning default role 'donor' and status 'active'.`
          );
        }
    ...
    app.put("/set-user-role/:uid", verifyFirebaseToken, async (req, res) => {
      try {
        ...
        if (dbUser?.role !== "admin") {
          console.warn(
            `Unauthorized role update attempt by UID: ${userFromToken.uid} with role: ${dbUser?.role}`
          );
          return res
            .status(403)
            .send({ message: "Forbidden: Only admins can change user roles." });
        }
        ...
        if (!userToUpdateUid || !role || !status) {
          return res
            ...
            .send({
              message:
                "Bad Request: Missing UID, role, or status in the request.",
            });
        }
        ...
        if (userToUpdateUid === userFromToken.uid) {
          return res
            ...
            .send({
              message:
                "Forbidden: Admins cannot change their own role or status.",
            });
        }
        ...
        if (result.matchedCount === 0) {
          return res
            .status(404)
            .send({ message: "User not found for update." });
        }

        res
          .status(200)
          .send({
            message: `Successfully updated role for user ${userToUpdateUid} to ${role} and status to ${status}.`,
          });
      } catch (error) {
    ...
    app.put(
      ...
      async (req, res) => {
        try {
          ...
          if (dbUser?.role !== "admin") {
            console.warn(
              `Unauthorized status update attempt by UID: ${userFromToken.uid} with role: ${dbUser?.role}`
            );
            return res
              .status(403)
              .send({
          ...
          if (!userToUpdateId || !newStatus) {
            return res
              .status(400)
              .send({ message: "Bad Request: Missing user ID or new status." });
          }
          ...
          if (newStatus !== "active" && newStatus !== "blocked") {
            return res
              ...
              .send({
                message:
                  "Bad Request: Status must be either 'active' or 'blocked'.",
              });
          }
          ...
          if (userToUpdateId === dbUser._id.toString()) {
            return res
              .status(403)
              .send({
                message: "Forbidden: Admins cannot change their own status.",
              });
          }
          ...
          res
            .status(200)
            .send({
              message: `Successfully updated status for user ${userToUpdateId} to ${newStatus}.`,
            });
        } catch (error) {
    ...
    app.post("/post-blog", verifyFirebaseToken, async (req, res) => {
      try {
        ...
        if (dbUser?.role !== "admin" && dbUser?.role !== "volunteer") {
          console.warn(
            `Unauthorized blog post attempt by UID: ${userFromToken.uid} with role: ${dbUser?.role}`
          );
          return res
            .status(403)
            .send({
              message:
                "Forbidden: Only admins and volunteers can create blog posts.",
            });
        }
        ...
        res
          .status(500)
          .json({
            message: "Failed to save new blog post",
            error: error.message,
          });
      }
    });
      ...
      async (req, res) => {
        try {
          ...
          if (
            ...
            console.warn(
              `Unauthorized status toggle attempt by UID: ${userFromToken.uid}`
            );
            return res
              .status(403)
              .send({
          ...
          const newStatus =
            blogPost.status === "published" ? "draft" : "published";
          ...
          if (result.matchedCount === 0) {
            return res
              .status(404)
              .send({
                message: "Blog post not found or no changes were made.",
              });
          }
    ...
    app.delete("/delete-blog/:id", verifyFirebaseToken, async (req, res) => {
      try {
        ...
        if (
          ...
          console.warn(
            `Unauthorized blog delete attempt by UID: ${userFromToken.uid}`
          );
          return res
            .status(403)
            .send({
        ...
        if (result.deletedCount === 0) {
          return res
            .status(404)
            .send({ message: "Blog post not found or already deleted." });
        }
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        try {
          ...
          console.error(
            "Backend: Error fetching recent donation requests:",
            error
          );
          res
            .status(500)
            .json({
              message: "Failed to fetch recent donation requests",
              error: error.message,
            });
        }
      }
    ...
    app.get(`/donationRequests/:id`, verifyFirebaseToken, async (req, res) => {
      ...
      try {
        ...
        console.error(
          "Backend: Error fetching single donation request:",
          error
        );
        res
          .status(500)
      ...
      async (req, res) => {
        ...
        if (userIdFromParams !== userIdFromToken) {
          return res
            ...
            .send({
              message:
                "Forbidden: You can only view your own donation history.",
            });
        }

        try {
          const myDonations = await requestCollection
            .find(userIdFromParams ? { uid: userIdFromParams } : {})
          ...
          res
            .status(500)
            .send({
              message: "Failed to fetch your donation history",
              error: error.message,
            });
        }
      }
    ...
    app.get("/all-donation-requests", verifyFirebaseToken, async (req, res) => {
      try {
        const allDonationRequests = await requestCollection
          .find({})
          .sort({ createdAt: -1 })
          .toArray();
    ...
    app.get(
      ...
      async (req, res) => {
        try {
          ...
          console.error(
            "Error fetching total requests count for admin dashboard:",
            error
          );
          res
            .status(500)
            .send({
              message: "Failed to fetch total requests count",
              error: error.message,
            });
        }
      }
        ...
        try {
          ...
          if (!isOwner && !isAdmin) {
            ...
            return res
              ...
              .json({
                message:
                  "Forbidden: You do not have permission to edit this request.",
              });
          }
          ...
          if (result.matchedCount === 0) {
            return res
              .status(404)
              .json({
                message: "Donation request not found or no changes were made.",
              });
          }
    ...
    app.put("/claimRequest/:id", verifyFirebaseToken, async (req, res) => {
      ...
      try {
        ...
        const donationRequest = await requestCollection.findOne({
          _id: new ObjectId(id),
        });
        ...
        if (
          ...
          return res
            .status(409)
            .json({
              message: `This request has already been claimed and is no longer available.`,
            });
        }
        ...
        if (result.matchedCount === 0) {
          ...
          return res
            ...
            .json({
              message:
                "Failed to update request, not found after initial check.",
            });
        }
        ...
        res
          ...
          .json({
            message:
              "An internal server error occurred while updating the request.",
          });
      }
    });
...
app.get("/", (req, res) => {
  res.send("Hello! This is LifeStream Server");
});
